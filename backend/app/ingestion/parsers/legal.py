...
_RE_MUC = re.compile(r"^M[ụu]c\s+([IVXLCDM\d]+)\b(.*)$", re.IGNORECASE)
_RE_DIEU = re.compile(r"^\s*Đi[eề]u\s+(\d+)\s*[\.\:]?\s*(.*)$", re.IGNORECASE)
_RE_KHOAN = re.compile(r"^\s*(\d+)\s*[\.\)]\s+(.*)$")
_RE_KHOAN_WORD = re.compile(r"^\s*Kho[ảa]n\s+(\d+)\s*[.:]?\s*(.*)", re.IGNORECASE)
_RE_DIEM = re.compile(r"^\s*([a-zđ])\s*\)\s*(.*)$", re.IGNORECASE)
_RE_DIEM_DOT = re.compile(r"^\s*([a-zđ])\s*\.\s+(.*)", re.IGNORECASE)
...
def parse_structure(blocks: List[Block]) -> List[Provision]:
    ...
    for blk in blocks:
        ...
        m = _RE_MUC.match(text)
        if m:
            flush()
            chapter = "Mục " + m.group(1).strip()
            article = article_title = clause = point = None
            current = None
            continue
        ...
        m = _RE_DIEM_DOT.match(text)
        # Điểm (dot form) must sit inside a Khoản; only treat "a. " as a point if
        # we already have a clause context, else it's just body text.
