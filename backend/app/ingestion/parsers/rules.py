...
def llm_extract_amendments(
    text: str,
    doc_number: Optional[str] = None,
    source_page: Optional[int] = None,
    max_chars: int = 6000,
) -> list[AmendmentProposal]:
    """Dùng LLM trích amendment proposals dạng JSON có evidence span.

    Chỉ gọi khi regex không đủ (vd: văn bản có cách diễn đạt không chuẩn).
    Kết quả có confidence thấp hơn regex và cần CHANGE_EVENT_REVIEW.
    """
    try:
        from llm.client import get_client
    except Exception:
    ...
    prompt = (
        "Tìm TẤT CẢ điều khoản SỬA ĐỔI trong văn bản sau. "
        "Trả về JSON array, mỗi phần tử gồm:\n"
        "  operation: REPLACE_TEXT|REPLACE_PROVISION|INSERT_AFTER|DELETE_TEXT|REPEAL_PROVISION\n"
        "  target_document_number: số hiệu văn bản bị sửa (nếu có)\n"
        "  target_article: số Điều bị sửa\n"
        "  target_clause: số Khoản bị sửa (nếu có)\n"
        "  old_text: nội dung CŨ bị thay (nếu có)\n"
        "  new_text: nội dung MỚI thay thế (nếu có)\n"
        "  valid_from: ngày hiệu lực (YYYY-MM-DD, nếu có)\n"
        "  evidence_text: đoạn văn bản nguồn chứa thông tin này\n"
        "  confidence: 0.0-1.0\n"
    ...
    try:
        data = get_client().complete_json("", prompt)
    except Exception:
        return []

    if not isinstance(data, list):
        data = data.get("amendments", []) if isinstance(data, dict) else []
    ...
    for item in data:
        if not isinstance(item, dict):
            continue
        try:
            op_str = item.get("operation", "REPLACE_TEXT")
            op = AmendmentOperation(op_str)
        except ValueError:
            op = AmendmentOperation.REPLACE_TEXT

        vf = None
        if item.get("valid_from"):
            vf = normalize_date(str(item["valid_from"]))

        evidence = None
        if item.get("evidence_text"):
            evidence = EvidenceSpan(
                page=source_page or 0,
                text=str(item["evidence_text"])[:500],
            )

        proposals.append(AmendmentProposal(
            operation=op,
            target_document_number=item.get("target_document_number"),
