...
def activate_change_event(session, change_event_id: str,
    ...
    from ingestion.legal_extract import extract_obligation, extract_scope
    v2_obligation = extract_obligation(result.new_content, source_provision=prov.provision_id)
    v2_scope = extract_scope(result.new_content)
    v2.obligation = v2_obligation.model_dump() if v2_obligation else None
    v2.scope = v2_scope.model_dump() if v2_scope else None
    session.add(v2)
