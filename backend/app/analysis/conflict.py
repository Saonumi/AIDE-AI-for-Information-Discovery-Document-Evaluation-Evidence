...
_HAS_MONEY_UNIT = re.compile(
    r"\b(đ[ồo]ng|vnd|vnđ|tri[ệe]u|t[ỷỉ]|ngh[ìi]n\s*đ|ng[àa]n\s*đ)\b",
    re.IGNORECASE,
)


def _has_monetary_context(text: str) -> bool:
    return bool(_HAS_MONEY_UNIT.search(text or ""))
...
def _rule_conflict(oa: Dict[str, Any], ob: Dict[str, Any], ca: str, cb: str):
    ...
    # Only parse money when the text contains an explicit monetary unit — prevents
    # article/clause references like "Điều 7 Khoản 2" from being mis-parsed as VND values.
    va = oa.get("value_normalized")
    if va is None and _has_monetary_context(oa.get("value") or ""):
        va = money_to_vnd(oa.get("value") or "")

    vb = ob.get("value_normalized")
    if vb is None and _has_monetary_context(ob.get("value") or ""):
        vb = money_to_vnd(ob.get("value") or "")
